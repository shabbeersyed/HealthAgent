...
def stop_recording_route():
    try:
        ...
        # ✅ Keep original summary for PDF
        pdf_summary = summary  

        # ✅ Create clean version for frontend readability
        clean_summary = summary.replace("{", "").replace("}", "").replace("[", "").replace("]", "")
        clean_summary = clean_summary.replace('"', "").replace("'", "")
        clean_summary = clean_summary.replace(",", "\n•")  # Converts commas to bullet points

        # PDF (uses original structured summary)
        pdf_filename = f"SOAP_Report_{uuid.uuid4().hex}.pdf"
        pdf_path = os.path.join(SOAP_FOLDER, pdf_filename)
        create_pdf(pdf_summary, patient_data, pdf_path)

        # ✅ Send cleaned summary to frontend
        return jsonify({"success": True, "summary": clean_summary, "pdf_path": f"SOAP/{pdf_filename}"})
    except Exception as e:
        print("❌ Error:", e)
        return jsonify({"success": False, "error": str(e)}), 500
...
@app.route("/")
def home():
    return "✅ HealthAgent API is running successfully on Render!"

# === Run Flask ===
if __name__ == "__main__":
    print("🚀 Flask server running on Render at port 10000")
